...
// SignAssertion signs the <saml:Assertion> element inside the given XML document
// using enveloped RSA-SHA256 + Exclusive C14N.
func SignAssertion(doc *etree.Document, kp *IdPKeyPair) error {
	...
	// Step 1: Canonicalize assertion (without any existing Signature) for digest
	...
	// Step 3: Canonicalize SignedInfo and sign
	...
	// Step 4: Build Signature element
	sigEl := buildSignatureElement(signedInfo, sigBytes, kp.CertDER)

	// Step 5: Insert Signature right after Issuer in Assertion
...
func insertAfterIssuer(assertion *etree.Element, sig *etree.Element) {
	...
	// InsertChild inserts sig before the next sibling element
	assertion.InsertChild(children[issuerIdx+1], sig)
}

// canonicalize performs Exclusive XML Canonicalization (exc-c14n) on an element.
// This is a simplified but SAML-compatible implementation.
...
func c14nElement(buf *strings.Builder, el *etree.Element, parentNS []nsEntry) error {
	...
	// Merged namespace context for children
	mergedNS := mergeNS(parentNS, visibleNS)

	// Child nodes: text and elements in document order
...
// collectVisibleNS returns namespace declarations needed on this element
// that are not already in scope from the parent context.
func collectVisibleNS(el *etree.Element, parentNS []nsEntry) []nsEntry {
	...
	// Attribute namespaces (excluding xmlns declarations)
	...
	// Filter out those already declared by parent with same URI
	var result []nsEntry
