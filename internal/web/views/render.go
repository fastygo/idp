...
func RenderAdminConsole(w http.ResponseWriter, r *http.Request, cfg *config.Config, users []UserEntry) {
	locale := resolveLocale(r)
	l, _ := i18n.Load(locale)

	data := AdminPageData{
		Layout:  buildLayoutData(r, cfg, l.Common.Admin.Title),
		Strings: l.Common.Admin,
		Users:   users,
	}

	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	if err := AdminConsolePage(data).Render(r.Context(), w); err != nil {
		log.Printf("render admin console: %v", err)
		http.Error(w, "Internal error", http.StatusInternalServerError)
	}
}

func RenderMailer(w http.ResponseWriter, r *http.Request, cfg *config.Config, logEntries []MailLogEntry, flash string, flashError bool) {
	locale := resolveLocale(r)
	l, _ := i18n.Load(locale)

	data := MailerPageData{
		Layout:     buildLayoutData(r, cfg, l.Common.Mailer.Title),
		NavStrings: l.Common.Admin,
		Strings:    l.Common.Mailer,
		Log:        logEntries,
		Flash:      flash,
	...
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	if err := AdminMailerPage(data).Render(r.Context(), w); err != nil {
		log.Printf("render mailer: %v", err)
		http.Error(w, "Internal error", http.StatusInternalServerError)
	}
}
